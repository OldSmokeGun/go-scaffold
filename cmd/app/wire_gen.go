...
func initApp(rotateLogs *rotatelogs.RotateLogs, logLogger log.Logger, zapLogger *zap.Logger, configConfig *config2.Config, config3 *orm.Config, config4 *redis.Config, config5 *trace.Config, config6 *discovery.Config) (*app.App, func(), error) {
	db, cleanup2, err := orm.New(logLogger, zapLogger, config3)
	if err != nil {
		return nil, nil, err
	}
	client, cleanup3, err := redis.New(config4, logLogger)
	if err != nil {
		cleanup2()
		return nil, nil, err
	}
	...
	tracer, cleanup4, err := trace.New(config5, logLogger)
	if err != nil {
		cleanup3()
		cleanup2()
		return nil, nil, err
...
func initCommand(rotateLogs *rotatelogs.RotateLogs, logLogger log.Logger, zapLogger *zap.Logger, configConfig *config2.Config, config3 *orm.Config, config4 *redis.Config, config5 *trace.Config, config6 *discovery.Config) (*command.Command, func(), error) {
